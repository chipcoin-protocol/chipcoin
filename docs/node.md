...
Bootstrap discovery is optional and hint-only.

At runtime, the node may use one or more bootstrap discovery URLs to:

- fetch peer hints at startup
- refresh peer hints periodically
- announce its public P2P endpoint when `NODE_PUBLIC_HOST` is configured

Recommended discovery settings:

- `NODE_DIRECT_PEERS`
  - one or more known-good manual startup peers
- `NODE_BOOTSTRAP_URLS`
  - comma-separated bootstrap discovery base URLs
- `NODE_BOOTSTRAP_URL`
  - legacy single-URL compatibility fallback
- `NODE_PUBLIC_HOST`
  - public DNS name or IP that other nodes can dial
- `NODE_PUBLIC_P2P_PORT`
  - announced public P2P port, defaults to `NODE_P2P_BIND_PORT`
- `BOOTSTRAP_ANNOUNCE_ENABLED`
  - enable or disable announce heartbeats
- `BOOTSTRAP_REFRESH_INTERVAL_SECONDS`
  - periodic peer refresh interval

Bootstrap discovery precedence is deterministic:

1. `NODE_DIRECT_PEERS`, then `NODE_DIRECT_PEER`
2. `NODE_BOOTSTRAP_URLS`
3. `NODE_BOOTSTRAP_URL`
4. shared fallbacks `DIRECT_PEERS`, `DIRECT_PEER`, `BOOTSTRAP_URLS`, `BOOTSTRAP_URL`

When both multi-URL and single-URL bootstrap settings are present:

- `*_URLS` entries are tried first, in the configured order
- `*_URL` is appended afterward as a trailing compatibility fallback
- role-specific `NODE_*` settings outrank shared fallback settings

Legacy compatibility:

- `NODE_BOOTSTRAP_URL` remains supported intentionally for older deployments
- shared `BOOTSTRAP_URL` remains supported only as a fallback when `NODE_*` discovery vars are unset
- new configurations should prefer `NODE_BOOTSTRAP_URLS`

Failure behavior:

- bootstrap fetch failure only logs a warning
- bootstrap announce failure only logs a warning
- node startup continues with manual peers, persisted peers, or inbound peers

Bootstrap remains non-authoritative:

- discovered peers are only connection hints
- the node still validates all peer traffic and chain data normally
